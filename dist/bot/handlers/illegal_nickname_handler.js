...
async function illegalNicknameHandler(member) {
    ...
    if (!member.manageable)
        return; // the user has a higher role than the bot, so don't continue
    ...
    if (!non_allowed_occurrences_threshold_exceeded)
        return; // only proceed if the non-allowed character threshold is exceeded
...
//# sourceMappingURL=illegal_nickname_handler.js.map