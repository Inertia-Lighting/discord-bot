...
async function interactionHandler(unknown_interaction) {
    ...
    const client_interaction = client.$.interactions.find(client_interaction => {
        const unknown_interaction_identifier = unknown_interaction.type === 'MESSAGE_COMPONENT' ? (unknown_interaction.customId) : (unknown_interaction.type === 'APPLICATION_COMMAND' ? (unknown_interaction.commandName) : (unknown_interaction.type === 'APPLICATION_COMMAND_AUTOCOMPLETE' ? (unknown_interaction.commandName) : (unknown_interaction.type === 'PING' ? (unknown_interaction.id) : (null))));
    ...
    if (!client_interaction) {
        ...
        if (unknown_interaction.type === 'APPLICATION_COMMAND' &&
