...
const { Timer, array_chunks, string_ellipses } = require('../../utilities');
const { createNoteForUser, updateNoteForUser, removeNoteFromUser, purgeNotesFromUser, lookupNotesForUser, lookupNoteForUser } = require('../handlers/user_notes_handler');
//---------------------------------------------------------------------------------------------------------------//
/**
 * Create note subcommand
 * @param {Discord.Message} message
 */
async function createNoteCommand(message) {
 ...
 * @param {Discord.Message} message
 */
async function updateNoteCommand(message) {
    const sub_command_args = message.content.split(/\s+/g).slice(2);
    const lookup_query = (sub_command_args[0] ?? '').replace(/[^\d\w\-]/g, '');
 ...
 * @param {Discord.Message} message
 */
async function removeNoteCommand(message) {
    const sub_command_args = message.content.split(/\s+/g).slice(2);
    const lookup_query = (sub_command_args[0] ?? '').replace(/[^\d\w\-]/g, '');
    if (lookup_query.length === 0) {
        message.reply({
 ...
 * @param {Discord.Message} message
 */
async function purgeNotesCommand(message) {
    const sub_command_args = message.content.split(/\s+/g).slice(2);
    const lookup_query = (sub_command_args[0] ?? '').replace(/[^\d\w\-]/g, '');
    try {
        await client.users.fetch(lookup_query);
 ...
 * @param {Discord.Message} message
 * @param {'user_mention'|'note_id'} lookup_method
 */
async function lookupNotesCommand(message, lookup_method) {
    const sub_command_args = message.content.split(/\s+/g).slice(2);
    const lookup_query = (sub_command_args[0] ?? '').replace(/[^\d\w\-]/g, '');
    ...
    await Timer(500);
    const user_notes = lookup_method === 'user_mention' ? (await lookupNotesForUser({
        discord_user_id: lookup_query,
    })) : ([
    ...
    message_button_collector.on('collect', async (button_interaction) => {
        ...
        if (message_button_collector.ended)
            return;
        await editEmbedWithNextUserNotesChunk();
    });
