...
module.exports = {
    ...
    async execute(message, args) {
        ...
        const commands_visible_to_user = client.$.commands.filter(cmd => user_permission_level >= cmd.permission_level).sort((a, b) => a.permission_level - b.permission_level);
        ...
        if (specified_command_alias?.length > 0) {
            ...
            const commands_visible_to_user_with_prefix = commands_visible_to_user.map(command => command.aliases.map(command_alias => `${command_prefix}${command_alias.replace('#{cp}', `${command_prefix}`)}`).join(' | '));
            message.channel.send({
                embeds: [
                    new Discord.MessageEmbed({
                        color: 0x60A0FF,
