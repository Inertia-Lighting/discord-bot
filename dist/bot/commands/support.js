...
async function createSupportTicketChannel(guild, guild_member, support_category) {
    ...
    if (potential_open_ticket_channel)
        throw new Error('A support ticket channel is already open!');
    const support_ticket_channel = await guild.channels.create(support_channel_name, {
        ...
        permissionOverwrites: [
            ...support_tickets_category.permissionOverwrites.cache.values(),
            {
...
async function createTranscriptForSupportTicket(support_ticket_channel) {
    ...
    do {
        ...
        if (before_message_id && oldest_message_id === before_message_id)
            break;
    ...
    if (!support_ticket_owner_id)
        throw new Error('Unable to find the support ticket owner id!');
    return {
        metadata: {
            channel: {
        ...
        async function supportTicketCommand() {
            ...
            category_selection_menu_interaction_collector.on('collect', async (category_selection_menu_interaction) => {
                ...
                category_instructions_message_components_collector.on('collect', async (interaction) => {
                    ...
                    switch (interaction.customId) {
                        case 'ready_for_support_staff': {
                            ...
                            await support_channel.permissionOverwrites.set([
                                ...support_channel.permissionOverwrites.cache.values(),
                    ...
                    if (notice_to_press_button_message)
                        notice_to_press_button_message.delete().catch(console.warn);
