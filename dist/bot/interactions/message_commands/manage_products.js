...
module.exports = {
    ...
    async execute(interaction) {
        const db_roblox_products = (await mongo_js_1.go_mongo_db.find(process.env.MONGO_DATABASE_NAME, process.env.MONGO_PRODUCTS_COLLECTION_NAME, {}));
        if (interaction.isAutocomplete()) {
            ...
            for (const db_roblox_product of filtered_db_roblox_products) {
                mapped_db_roblox_products.push({
                    ...db_roblox_product,
                    similarity_score: (0, string_similarity_1.compareTwoStrings)(search_query, db_roblox_product.code),
                });
            }
            const matching_db_roblox_products = mapped_db_roblox_products.sort((a, b) => b.similarity_score - a.similarity_score).sort((a, b) => {
                ...
                if (a.code.startsWith(first_char_of_query))
                    return -1;
                if (b.code.startsWith(first_char_of_query))
                    return 1;
                return 0;
            }).filter(({ similarity_score, code }, index) => search_query.length > 0 ? (similarity_score >= 0.25 || (similarity_score < 0.25 && index < 10)) : true);
            if (matching_db_roblox_products.length === 0) {
                return interaction.respond([]);
            }
            interaction.respond(matching_db_roblox_products.slice(0, 5).map(db_roblox_product => ({
...
//# sourceMappingURL=manage_products.js.map