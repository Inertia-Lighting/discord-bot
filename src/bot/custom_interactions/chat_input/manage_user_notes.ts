...
import { Timer, array_chunks, getMarkdownFriendlyTimestamp, string_ellipses } from '@root/utilities';
...
import { UserNote, createNoteForUser, lookupNoteForUser, lookupNotesForUser, purgeNotesFromUser, removeNoteFromUser, updateNoteForUser } from '@root/bot/handlers/user_notes_handler';
        ...
        embeds: [
            CustomEmbed.from({
                ...
                description: [
                    ...
                    '**Content**',
                    '\`\`\`',
                    `${string_ellipses(Discord.escapeMarkdown(user_note.record.note), 512)}`,
                    '\`\`\`',
                ].join('\n'),
            }),
        ],
