...
export default new CustomInteraction({
    ...
    handler: async (discord_client, interaction) => {
        ...
        const selection = interaction.options.getString('properties', false) as
            | 'general'
            | 'transactions'
            | 'transfers'
            | 'punishments';
        ...
        const userData = await prisma.user.findFirst({
            where: {
                OR: [{ discordId: user_id }, { robloxId: user_id }],
            },
            include: includeOptions,
        });
        const blacklistData = userData?.receivedPunishments.some(
            (punishment) => punishment.punishmentType === 'blacklist',
        );
