...
if (user_verification_endpoints_base64_encoded_token.length < 1)
    throw new Error(
        'Environment variable: API_BASE64_ENCODED_TOKEN_FOR_USER_VERIFICATION_ENDPOINTS; is not set correctly.',
    );
...
async function userAlreadyVerifiedHandler(interaction: Discord.ChatInputCommandInteraction): Promise<void> {
    await interaction
        .editReply({
            embeds: [
                CustomEmbed.from({
...
async function verifyHandler(interaction: Discord.ChatInputCommandInteraction): Promise<void> {
    ...
    const fetch_pending_verification_response = await got
        .post(`https://${api_server}/v3/user/verification/context/fetch`, {
            headers: {
    ...
    const maybe_verification_code = (fetch_pending_verification_response as unknown as { verification_code?: unknown })
        .verification_code;
    ...
    try {
        ...
        await got
            .post(`https://${api_server}/v3/user/verification/context/submit`, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `InertiaAuthUserVerificationEndpoints ${user_verification_endpoints_base64_encoded_token}`,
                },
        ...
        try {
            ...
            if (typeof unknownErr === 'object' && unknownErr !== null) {
                ...
                else if ((unknownErr as { message?: unknown }).message)
                    debugInfo = String((unknownErr as { message?: unknown }).message);
            }
        } catch {
                ...
                embeds: [
                    CustomEmbed.from({
                        ...
                        description: [
                            ...
                            ...(debugInfo ? ['', 'Debug Information:', `\`\`\`js\n${debugInfo}\n\`\`\``] : []),
                        ].join('\n'),
                    }),
                ],
            })
...
export default new CustomInteraction({
    ...
    handler: async (discord_client, interaction) => {
        ...
        const request = await got
            .post(`https://${api_server}/v3/user/identity/fetch`, {
                json: {
                    discordId: interaction.user.id,
                },
                throwHttpErrors: false,
