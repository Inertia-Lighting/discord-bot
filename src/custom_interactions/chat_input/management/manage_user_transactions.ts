...
async function manageProductsAutocompleteHandler(interaction: Discord.AutocompleteInteraction): Promise<void> {
    ...
    const filtered_db_roblox_products = db_roblox_products.filter((db_roblox_product) => {
        const user_owns_product = db_user_data.transactions.some((t) => t.productCode === db_roblox_product.code);
    ...
    const matching_db_roblox_products = mapped_db_roblox_products
    ...
    const autocomplete_results = matching_db_roblox_products.slice(0, 5).map((db_roblox_product) => ({
...
async function manageProductsChatInputCommandHandler(interaction: Discord.ChatInputCommandInteraction): Promise<void> {
    ...
    switch (product_code) {
        case ALL_PRODUCTS_CODE: {
            product_codes_to_manage.push(
                ...db_roblox_products
                    ...
                    .map((db_roblox_product) => db_roblox_product.code),
            );
        ...
        case ALL_VIEWABLE_PRODUCTS_CODE: {
            product_codes_to_manage.push(
                ...db_roblox_products
                    .filter((db_roblox_product) => db_roblox_product.viewable)
                    .filter((p) => {
                        const user_owns_product = db_user_data.transactions.some((t) => t.productCode === p.code);
                        return (
                            (action_to_perform === ManageTransactionsAction.Add && !user_owns_product) ||
    ...
    try {
        ...
        await logging_channel.send({
            embeds: [
                CustomEmbed.from({
                    color:
                        action_to_perform === ManageTransactionsAction.Add
                    ...
                    description: [
                        ...
                        ...product_codes_to_manage.map((product_code_to_manage) => `- \`${product_code_to_manage}\``),
