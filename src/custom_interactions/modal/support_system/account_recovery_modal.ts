...
export default new CustomInteraction({
    ...
    handler: async (discord_client, interaction) => {
        ...
        try {
            await supportSystemManager.handleModalSubmission(interaction, SupportCategoryId.Recovery);

            const modalReplyMessage = await interaction.editReply({
                content: ['You selected Account Recovery.', 'Go to your support ticket channel to continue.'].join(
                    '\n',
                ),
            });
