...
export default new Interaction({
    ...
    data: {
        ...
        options: [
            ...
            //     name: 'reason',
            //     description: 'Why is this user getting banned?',
            //     required: true,
            //     choices: [
            //         { 
            //             name: 'Reason 1',
        ...
        user.member.send({
            embeds: [
                CustomEmbed.from({
                    ...
                    title: 'baned',
                    description: `You were baned from Inertia Lighting for: \n ${reason}`
                })
            ]
        })
        user.member.ban({reason: reason, deleteMessageSeconds: 60 *  60 * 24})
        } catch(err) {
                interaction.editReply({
                    embeds: [
                        CustomEmbed.from({
