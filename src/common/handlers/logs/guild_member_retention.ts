...
function discordTimestampsDifferenceInDays(
    ...
    return Math.floor((parsed_newest_timestamp - parsed_oldest_timestamp) / (60 * 60 * 24));
}
...
async function guildMemberAddLogger(member: Discord.GuildMember) {
    ...
    if (!member_retention_logging_channel.isTextBased())
        throw new TypeError('member_retention_logging_channel is not a text channel');
    if (!member_retention_logging_channel.isSendable())
        throw new Error('member_retention_logging_channel is not sendable!');
    ...
    await member_retention_logging_channel
        .send({
            embeds: [
                CustomEmbed.from({
...
async function guildMemberBannedLogger(guild_ban: Discord.GuildBan) {
    if (!(guild_ban instanceof Discord.GuildBan))
        throw new TypeError('guildMemberBannedLogger(): ban is not a GuildBan');
...
export { guildMemberAddLogger, guildMemberBannedLogger, guildMemberRemoveLogger };
