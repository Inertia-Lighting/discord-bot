...
export async function userProfileHandler(
            ...
            .editReply({
                embeds: [
                    CustomEmbed.from({
                        ...
                        title: 'Unknown User',
                        description: ["That user doesn't exist in our database!"].join('\n'),
                    }),
                ],
            })
            .catch(console.warn);
    ...
    const product_map = db_user.transactions
        ...
        .filter((productCode) =>
            db_products.some((db_product) => db_product.viewable && db_product.code === productCode),
        );
    ...
    if (discord_member) {
        addProductsByCondition(
            !!discord_member.premiumSince,
            db_products.filter((product) => product.supporter_perk),
        );
