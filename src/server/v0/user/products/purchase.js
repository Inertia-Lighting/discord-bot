...
module.exports = (router, client) => {
    router.post('/v0/user/products/purchase', async (req, res) => {
        ...
        if (db_user_data.products[db_roblox_product_data.old_code]) {
        ...
        await go_mongo_db.update(process.env.MONGO_OLD_DATABASE_NAME, process.env.MONGO_OLD_USERS_COLLECTION_NAME, {
            ...
            $set: {
                [`products.${db_roblox_product_data.old_code}`]: true,
            },
        });
        ...
        user_purchases_logging_channel?.send(new Discord.MessageEmbed({
            ...
            description: [
                ...
                `**Bought product:** \`${db_roblox_product_data.old_code}\``,
            ].join('\n'),
        }))?.catch(console.warn);
